...
function EmpresaDocsModal({ api, mandanteId, empresa, perms = {}, isSuper, onClose }) {
  ...
  const load = useCallback(() => {
    api(`/mandantes/${mandanteId}/empresa-acreditacion?empresa_id=${empresa.empresa_id}`).then((d) => setDetalle(d.detalle || [])).catch((e) => toast.error(e.message));
  }, [api, mandanteId, empresa]);
  useEffect(() => { load(); }, [load]);
...
function MandanteDetail({ api, id, onBack, openDetail, canManage, isSuper, perms = {} }) {
