...
// fetch con timeout (evita cuelgues indefinidos si el host de Supabase no es alcanzable
// desde el clúster de producción). Un cuelgue aquí bloqueaba el arranque y causaba 520.
...
export async function authSignIn(email, password) {
  const r = await fetchT(`${URL_}/auth/v1/token?grant_type=password`, {
    method: 'POST',
...
export async function adminCreateUser(email, password, user_metadata = {}) {
  const r = await fetchT(`${URL_}/auth/v1/admin/users`, {
    method: 'POST',
...
export async function adminDeleteUser(id) {
  const r = await fetchT(`${URL_}/auth/v1/admin/users/${id}`, {
    method: 'DELETE',
...
export async function adminFindUserByEmail(email) {
  const r = await fetchT(`${URL_}/auth/v1/admin/users?page=1&per_page=200`, {
    headers: { apikey: SECRET, Authorization: `Bearer ${SECRET}` },
  });
...
// Decodifica el payload del JWT (sin verificar firma) para leer exp/sub.
...
export async function getAuthUser(token) {
  ...
  const r = await fetchT(`${URL_}/auth/v1/user`, {
    headers: { apikey: PUB, Authorization: `Bearer ${token}` },
    cache: 'no-store',
  });
  if (!r.ok) return null;
  const u = await r.json();
  // Cachear hasta la expiración real del token (menos un margen), en vez de 60s fijos.
  // Así se evita un round-trip de red a Supabase en cada navegación.
  const payload = decodeJwt(token);
  const tokenExpMs = payload?.exp ? payload.exp * 1000 - 30000 : 0; // 30s de margen
  const maxCap = Date.now() + 15 * 60 * 1000; // tope de 15 min por seguridad
  const exp = Math.min(tokenExpMs > Date.now() ? tokenExpMs : Date.now() + 60000, maxCap);
  tokenCache.set(token, { user: u, exp });
  // Limpieza best-effort para evitar crecimiento indefinido del cache.
...
export async function ensureBucket(id = BUCKET) {
  try {
    const r = await fetchT(`${URL_}/storage/v1/bucket`, {
      method: 'POST',
      headers: { apikey: SECRET, Authorization: `Bearer ${SECRET}`, 'Content-Type': 'application/json' },
...
export async function storageUpload({ path, bytes, contentType, bucket = BUCKET }) {
  ...
  const r = await fetchT(`${URL_}/storage/v1/object/${bucket}/${enc}`, {
    method: 'POST',
...
export async function storageDelete(path, bucket = BUCKET) {
  const enc = path.split('/').map(encodeURIComponent).join('/');
  try {
    await fetchT(`${URL_}/storage/v1/object/${bucket}/${enc}`, {
      method: 'DELETE',
      headers: { apikey: SECRET, Authorization: `Bearer ${SECRET}` },
    });
  } catch (e) { /* noop: storage cleanup is best-effort */ }
}

export async function storageSignedUrl(path, expiresIn = 900, bucket = BUCKET) {
  const enc = path.split('/').map(encodeURIComponent).join('/');
  const r = await fetchT(`${URL_}/storage/v1/object/sign/${bucket}/${enc}`, {
    method: 'POST',
    headers: { apikey: SECRET, Authorization: `Bearer ${SECRET}`, 'Content-Type': 'application/json' },
