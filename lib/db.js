...
create table if not exists mandantes (
  ...
  razon_social text not null, rut text unique not null,
  direccion text, region text, comuna text,
  activo boolean not null default true,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), deleted_at timestamptz
);
...
create unique index if not exists uq_trab_mandante_activo on trabajador_asignaciones (trabajador_id, mandante_id) where estado = 'activo';
...
create table if not exists categorias_documentales (
  ...
  nombre text not null, orden int not null default 0, activo boolean not null default true,
  created_at timestamptz not null default now()
);
