...
export const pool =
  ...
  new Pool({
    ...
    // Timeouts para que la app NUNCA se cuelgue indefinidamente si la DB/pooler
    // no es alcanzable desde el clúster (esto causaba el 520 en producción).
...
export function ensureSchema() {
  // Tope de tiempo global: si init() se cuelga (DB/Storage inalcanzable), rechaza
  // rápido y permite reintentar en el próximo request en vez de bloquear readiness.
  if (!schemaPromise) schemaPromise = withTimeout(init(), 30000, 'ensureSchema').catch((e) => { schemaPromise = null; throw e; });
...
create table if not exists desvinculaciones (
  desvinculacion_id uuid primary key default gen_random_uuid(),
  trabajador_id uuid,
  asignacion_id uuid,
  contrato_id uuid,
  empresa_id uuid,
  mandante_id uuid,
  rut text, nombre text, cargo text,
  contrato_numero text, empresa_nombre text, mandante_nombre text,
  tipo text not null default 'finiquito',
  causal text,
  bucket text, path text, nombre_archivo text, mime text, tamano int,
  creado_por uuid,
...
create table if not exists usuario_mandantes (
...
create table if not exists vehiculos (
  ...
  num_motor text, num_chasis text, numero_interno text,
  estado text not null default 'activo' check (estado in ('activo','inactivo')),
  observaciones text,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now(), deleted_at timestamptz
);
...
create table if not exists categorias_documentales (
  ...
  mandante_id uuid not null references mandantes(mandante_id) on delete cascade,
  tipo_recurso text not null check (tipo_recurso in ('empresa','trabajador','vehiculo','equipo','contrato')),
...
async function init() {
  ...
  // Operador/conductor de vehículos y equipos + disponibilidad manual
  await pool.query(`create table if not exists recurso_operadores (
    id uuid primary key default gen_random_uuid(),
    recurso_tipo text not null check (recurso_tipo in ('vehiculo','equipo')),
    recurso_id uuid not null,
    trabajador_id uuid not null references trabajadores(trabajador_id) on delete cascade,
  ...
  await pool.query('create index if not exists ix_recop_recurso on recurso_operadores (recurso_tipo, recurso_id)').catch(() => {});
  await pool.query("alter table vehiculos add column if not exists estado_operativo text not null default 'disponible'").catch(() => {});
  await pool.query("alter table equipos add column if not exists estado_operativo text not null default 'disponible'").catch(() => {});
  // Control de acceso (Visor / portería con lector 2D de PDA)
  ...
  await pool.query('create index if not exists ix_accesos_created on accesos (created_at desc)').catch(() => {});
  await pool.query('create index if not exists ix_accesos_trab on accesos (trabajador_id, created_at desc)').catch(() => {});
  // Puntos de control de acceso (visores) + columnas de contexto en accesos
  await pool.query(`create table if not exists puntos_acceso (
    id uuid primary key default gen_random_uuid(),
  ...
  await pool.query('alter table puntos_acceso add column if not exists slug text').catch(() => {});
  await pool.query('create unique index if not exists ux_puntos_slug on puntos_acceso (slug)').catch(() => {});
  // Backfill de slugs legibles para puntos existentes sin slug
  try {
    const slugify = (s) => (s || '').toString().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60) || 'punto';
    const { rows: sinSlug } = await pool.query("select id, nombre from puntos_acceso where slug is null or slug = ''");
    for (const r of sinSlug) {
      let base = slugify(r.nombre); let slug = base; let n = 2;
      while ((await pool.query('select 1 from puntos_acceso where slug=$1', [slug])).rows.length) { slug = `${base}-${n++}`; }
      await pool.query('update puntos_acceso set slug=$1 where id=$2', [slug, r.id]);
    }
  } catch {}
  await pool.query('alter table accesos add column if not exists punto_id uuid').catch(() => {});
  await pool.query('alter table trabajadores add column if not exists es_spot boolean default false').catch(() => {});
  await pool.query('alter table accesos add column if not exists punto_nombre text').catch(() => {});
  await pool.query('alter table accesos add column if not exists marcado_at timestamptz').catch(() => {});
  await pool.query('alter table accesos add column if not exists auto boolean not null default false').catch(() => {});
  await pool.query('update accesos set marcado_at = created_at where marcado_at is null').catch(() => {});
  await enableRLS();
  ...
  // Asegura (idempotente, por-mandante) el estándar documental de empresa para todos los mandantes
  await seedEmpresaReqs();
}
...
// Estándar documental a nivel de EMPRESA del grupo (por mandante). Todos sin vencimiento.
const EMPRESA_DOC_CATS = [
  { cat: 'Legal y Administrativa', reqs: ['Certificado de deuda tributaria', 'Certificado de pago de patente municipal al día', 'Certificado F30 (Cumplimiento de Obligaciones Laborales de la empresa)', 'Personería jurídica', 'Inicio de actividades SII', 'Dicom full', 'Inscripción SICEP', 'Curriculum empresa', 'Código de Conducta y Declaración de Conflictos de Interés'] },
  { cat: 'Reglamentos y Recepciones', reqs: ['Copia firmada de recepción de reglamento para empresas contratistas y subcontratistas', 'Copia firmada de recepción del Sistema de Gestión Integral de Salud y Seguridad del Trabajo (SGSST)', 'Reglamento Interno de Orden, Higiene y Seguridad', 'Recepción de S. Salud de RIOHS', 'Recepción de Dirección del Trabajo RIOHS'] },
  { cat: 'Prevención de Riesgos (SSOMA)', reqs: ['Política de Prevención de Riesgos', 'Certificado Mutualidad', 'Certificado de Accidentabilidad', 'Análisis de accidentes', 'Plan de mitigación', 'Aprobación líder APR del plan', 'Plan de Seguridad, Salud Ocupacional y Medio Ambiente que incluya todos los alcances definidos', 'Programa de Higiene y Salud Ocupacional, que incluya un levantamiento de riesgos higiénicos de acuerdo con la legislación vigente, identificando las áreas y posibles expuestos a los diferentes tipos de agente', 'Matriz de EPP por cargo', 'Plan de Emergencia'] },
];
// Crea (idempotente) las categorías y requisitos de empresa para un mandante si aún no los tiene.
export async function ensureEmpresaReqs(mandanteId) {
  const { rows } = await pool.query("select 1 from requisitos_documentales where mandante_id=$1 and tipo_recurso='empresa' limit 1", [mandanteId]);
  if (rows.length) return;
  let ordC = 0;
  for (const c of EMPRESA_DOC_CATS) {
    const catId = uuid();
    await pool.query('insert into categorias_documentales (categoria_id, mandante_id, tipo_recurso, nombre, orden) values ($1,$2,$3,$4,$5)', [catId, mandanteId, 'empresa', c.cat, ordC++]);
    let ordR = 0;
    for (const nombre of c.reqs) {
      await pool.query('insert into requisitos_documentales (requisito_id, mandante_id, tipo_recurso, categoria_id, nombre, obligatorio, tiene_vencimiento, dias_alerta, orden) values ($1,$2,$3,$4,$5,true,false,30,$6)', [uuid(), mandanteId, 'empresa', catId, nombre, ordR++]);
    }
  }
}
async function seedEmpresaReqs() {
  const mandantes = (await pool.query('select mandante_id from mandantes')).rows;
  for (const m of mandantes) { try { await ensureEmpresaReqs(m.mandante_id); } catch {} }
  console.log('[seed] empresa requisitos ensured');
}
